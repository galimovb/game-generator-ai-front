...
<template>
  ...
  >
    <div class="absolute top-2 right-2 text-xs text-muted-foreground z-10">{{ formatDate(game.createdAt) }}</div>
    ...
    <CardHeader>
      <div v-if="game.photos?.length" class="relative">
        <Carousel>
          ...
          <CarouselPrevious v-if="game.photos?.length > 1" class="absolute left-2 top-1/2 -translate-y-1/2" />
          <CarouselNext v-if="game.photos?.length > 1" class="absolute right-2 top-1/2 -translate-y-1/2" />
        </Carousel>
      </div>

      <!-- Иконка-заглушка когда нет фото -->
      <div v-else class="flex justify-center items-center w-full h-48 bg-muted rounded-lg border">
        <ImageIcon :size="48" class="text-muted-foreground" />
      </div>

      <CardTitle class="flex-1 mt-4">{{ game.title || 'Без названия' }}</CardTitle>
