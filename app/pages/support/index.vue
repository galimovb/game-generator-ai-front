 ...
 */
const messagesCache = ref<Map<number, {
  items: TicketMessage[]
...
const { data: tickets, pending: ticketsPending, refresh: refreshTickets } = await useAsyncData(
    'tickets',
    async () => {
      ...
      return response.result as { items: Ticket[]; pagination: { page: number; limit: number; total: number } }
    },
    {
      watch: [statusFilter, page, searchQuery],
      default: () => ({ items: [], pagination: { page: 1, limit: 20, total: 0 } }),
      lazy: true,
    }
)
...
const loadMessages = async (loadMore = false) => {
  ...
  try {
    const response = await api.get(`/tickets/${selectedTicketId.value}/messages`, {
 ...
 */
watch(currentMessagesCache, (cache) => {
 ...
 */
watch(selectedTicketId, async (newId, oldId) => {
...
const sendMessage = async () => {
  if ((!newMessage.value.trim() && selectedPhotos.value.length === 0) || !selectedTicketId.value || sending.value) return
  ...
  try {
    ...
    const response = await api.post(`/tickets/${selectedTicketId.value}/messages`, {
    ...
    <div class="grid gap-4 grid-cols-3 flex-1 min-h-0">
      <div class="rounded-md col-span-1 bg-muted/50 flex flex-col min-h-0">
        ...
        <div v-else class="overflow-y-auto flex-1">
            ...
            <div class="py-2 px-4">
              <div class="flex items-center gap-4">
                ...
                <div class="h-full text-right">
                  <Badge :variant="getStatusVariant(ticket.status)" class="gap-1">
                    <component
                        :is="statusConfig[ticket.status as keyof typeof statusConfig]?.icon"
                        class="h-3 w-3"
                    />
          ...
          <div v-if="!tickets?.items?.length" class="flex flex-col items-center justify-center gap-2 p-8 text-muted-foreground">
            <MessageSquare class="h-8 w-8" />
            <p class="text-sm text-center">
              {{ statusFilter !== 'all' ? 'По выбранному фильтру обращения не найдены' : 'У вас пока нет обращений' }}
            </p>
          </div>
        </div>

        <div v-if="tickets?.pagination?.total > limit" class="flex justify-center p-4 border-t flex-shrink-0">
          <Pagination
      ...
      <div class="col-span-2 rounded-md bg-muted/50 flex flex-col min-h-0">
        <template v-if="selectedTicket">
          <div class="border-b p-4 flex items-center justify-between flex-shrink-0 bg-muted/30 rounded-t-md">
            ...
            <Badge :variant="getStatusVariant(selectedTicket.status)" class="gap-1">
              <component :is="statusConfig[selectedTicket.status as keyof typeof statusConfig]?.icon" class="h-3 w-3" />
          ...
          <div
              ...
              @scroll="handleScroll"
          >
            <div v-if="currentMessagesCache?.loadingMore" class="flex justify-center py-2">
              <Loader2 :size="20" class="animate-spin text-muted-foreground" />
            </div>

            <div v-if="currentMessagesCache?.loading && !currentMessagesCache?.items.length" class="space-y-2">
              <div v-for="i in 7" :key="i" class="flex" :class="i % 2 === 0 ? 'justify-end' : 'justify-start'">
            ...
            <div v-else class="space-y-2">
              ...
              >
                ...
                <div
                    ...
                    :class="isMyMessage(message) ? 'bg-primary text-primary-foreground' : 'bg-background'"
                >
                  ...
                  <div v-if="message.photos?.length" class="mt-2 grid grid-cols-2 gap-2">
                    <img
              ...
              <div v-if="!currentMessagesCache?.items.length && !currentMessagesCache?.loading" class="flex flex-col items-center justify-center h-full">
                <MessageSquare class="h-12 w-12 mb-3 text-muted-foreground/50" />
                <p class="text-sm text-muted-foreground">Нет сообщений</p>
              </div>
            </div>
          </div>

          <div v-if="selectedTicket.status !== 'closed'" class="border-t p-4 flex-shrink-0 bg-muted/30 rounded-b-md">
            ...
            <div class="flex gap-2 items-end">
              ...
              <Button
                  @click="sendMessage"
                  :disabled="sending || (!newMessage.trim() && !selectedPhotos.length)"
                  size="icon"
              >
          ...
          <div v-else class="border-t p-4 text-center text-muted-foreground flex-shrink-0 bg-muted/30 rounded-b-md">
        ...
        <div v-else class="flex-1 flex items-center justify-center">
          <div class="text-center">
            <MessageSquare class="h-12 w-12 mx-auto mb-3 text-muted-foreground/50" />
            <h3 class="text-base font-medium mb-1 text-muted-foreground">Выберите обращение</h3>
            <p class="text-sm text-muted-foreground">Нажмите на обращение слева, чтобы открыть диалог</p>
          </div>
        </div>
      </div>
    </div>
