...
const selectedTicket = computed(() => {
  ...
  return tickets.value.items.find((t) => t.id === selectedTicketId.value) ?? null;
});
...
const loadMessages = async (loadMore = false) => {
  ...
  try {
    ...
    cache.pagination = { page: nextPage, limit: 50, total, hasMore: cache.items.length < total };
  } catch {
...
const changeStatus = async (status: string) => {
  ...
  actionLoading.value = true;
  try {
    const res = await api.post(`/tickets/${selectedTicketId.value}/status`, { status });
    updateTicketInList(res.result);
...
const changePriority = async (priority: string) => {
  ...
  actionLoading.value = true;
  try {
    const res = await api.post(`/tickets/${selectedTicketId.value}/priority`, { priority });
    updateTicketInList(res.result);
...
<template>
  <div class="flex-1 flex min-h-0 overflow-hidden" style="height: calc(100vh - 3.5rem)">
      <!-- Ticket list -->
          ...
          <div v-else>
            ...
            >
              <div class="flex items-start justify-between gap-2 mb-1">
                ...
                <Badge :variant="getStatusVariant(ticket.status)" class="text-xs flex-shrink-0">
              ...
              <div class="flex items-center justify-between">
                ...
                <span
                  ...
                  :class="priorityConfig[ticket.priority as keyof typeof priorityConfig]?.color"
                >
      ...
      <div class="flex-1 flex flex-col min-h-0 min-w-0">
        <template v-if="selectedTicket">
          ...
          <div class="border-b p-3 flex items-start justify-between flex-shrink-0 bg-muted/20">
            <div class="flex items-center gap-2">
              <Button variant="ghost" size="icon" class="h-7 w-7" @click="closeChat">
                <ArrowLeft :size="14" />
              </Button>
              <div>
            ...
            <div class="flex items-center gap-2 flex-shrink-0 flex-wrap justify-end">
              ...
              <DropdownMenu v-if="selectedTicket.status !== 'closed'">
                <DropdownMenuTrigger as-child>
                  ...
                  >
                    <component
                      :is="statusConfig[selectedTicket.status as keyof typeof statusConfig]?.icon"
                      :size="12"
                      :class="statusConfig[selectedTicket.status as keyof typeof statusConfig]?.color"
                    />
                ...
                <DropdownMenuContent align="end">
                  <DropdownMenuLabel class="text-xs">Изменить статус</DropdownMenuLabel>
              ...
              <DropdownMenu v-if="selectedTicket.status !== 'closed'">
                <DropdownMenuTrigger as-child>
                  ...
                  >
                    <span
                      :class="priorityConfig[selectedTicket.priority as keyof typeof priorityConfig]?.color"
                      class="font-medium"
                    >
                ...
                <DropdownMenuContent align="end">
                  <DropdownMenuLabel class="text-xs">Изменить приоритет</DropdownMenuLabel>
                  <DropdownMenuSeparator />
                  <DropdownMenuItem
                  ...
                  >
                    <span :class="cfg.color" class="font-medium">{{ cfg.label }}</span>
                  </DropdownMenuItem>
                </DropdownMenuContent>
              </DropdownMenu>
          ...
          >
            ...
            <div
              v-if="currentMessagesCache?.loading && !currentMessagesCache?.items.length"
              class="space-y-2"
            >
              <div
                v-for="i in 6"
            ...
            <div v-else class="space-y-2">
              <template v-for="message in currentMessagesCache?.items" :key="message.id">
                <!-- System message -->
                <div
              ...
              <div
                v-if="!currentMessagesCache?.items.length && !currentMessagesCache?.loading"
        ...
        <div v-else class="flex-1 flex items-center justify-center">
          <div class="text-center">
            <MessageSquare class="h-12 w-12 mx-auto mb-3 text-muted-foreground/30" />
