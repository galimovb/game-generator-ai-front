...
const features = [
  {
    ...
    description: "Опишите параметры — AI выдаст готовую игру с этапами и заданиями",
  },
  {
    icon: Users,
  ...
  },
  {
    ...
    description: "Выберите от спокойных настольных до максимально подвижных игр",
  },
  {
    icon: MapPin,
    title: "Любая площадка",
    description: "Зал, улица, лес, корпоративное пространство — с учётом размеров",
  },
  {
    icon: ListChecks,
    title: "Список реквизита",
    description: "Каждый сценарий включает полный список необходимого инвентаря",
  },
  {
    icon: BookOpen,
    ...
    <main class="flex-1">
      ...
      <section class="py-16 md:py-24 lg:py-32 px-4">
        <div class="container mx-auto text-center max-w-3xl">
          ...
          <div class="hero-badge inline-flex items-center gap-1.5 rounded-full border px-3 py-1 text-xs font-medium text-muted-foreground mb-6">
            <Flame :size="12" class="text-primary" />
            Генератор активных игр для детей
          </div>

          <h1 class="hero-title text-4xl md:text-5xl lg:text-6xl font-bold tracking-tight mb-6 leading-tight">
            Активные игры для детей —
            <span class="text-primary">за минуту</span>
          </h1>

          <p class="hero-text text-lg text-muted-foreground mb-10 max-w-2xl mx-auto">
            Для педагогов, аниматоров и вожатых. Укажите возраст и количество детей —
            получите готовый сценарий с этапами, заданиями и списком реквизита.
          </p>
      ...
      <section class="py-16 md:py-20 px-4">
        <div class="container mx-auto">
          <div class="text-center mb-12">
            <h2 class="scroll-fade text-2xl font-bold mb-3">Всё что нужно для игры</h2>
            <p class="scroll-fade text-muted-foreground" style="--stagger: 80ms">
        ...
        <div class="container mx-auto">
          <div class="scroll-fade flex items-center justify-between mb-10">
            <div>
              ...
              <p class="text-muted-foreground text-sm mt-1">Самые любимые сценарии сообщества</p>
            </div>
          ...
          <div v-else class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            >
              <CardHeader class="p-0">
                <div v-if="game.photos?.length" class="overflow-hidden rounded-t-xl">
                  <img
              ...
              <CardContent class="pt-4">
                <h3 class="font-semibold mb-1">{{ game.title || "Без названия" }}</h3>
                <p class="text-sm text-muted-foreground line-clamp-2 mb-3">{{ game.description || "Нет описания" }}</p>
                <div class="flex flex-wrap gap-2 text-xs text-muted-foreground">
                  <span class="bg-muted rounded-full px-2 py-0.5">{{ game.age }}+ лет</span>
                  <span class="bg-muted rounded-full px-2 py-0.5">{{ game.players }} чел</span>
                  <span class="bg-muted rounded-full px-2 py-0.5">{{ game.duration }} мин</span>
                </div>
              </CardContent>
            </Card>
          </div>
      ...
      <section class="py-16 md:py-20 px-4 border-t bg-muted/20">
        <div class="container mx-auto max-w-3xl">
          <div class="text-center mb-12">
            <h2 class="scroll-fade text-2xl font-bold mb-3">Как это работает</h2>
            <p class="scroll-fade text-muted-foreground" style="--stagger: 80ms">
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            >
              ...
              <div class="text-xs font-semibold text-primary uppercase tracking-widest mb-2">Шаг {{ step.n }}</div>
              <h3 class="font-semibold mb-2">{{ step.title }}</h3>
              <p class="text-sm text-muted-foreground leading-relaxed">{{ step.text }}</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section class="py-16 md:py-20 px-4 border-t">
        <div class="container mx-auto text-center max-w-xl">
          <h2 class="scroll-fade text-2xl font-bold mb-4">Попробуйте прямо сейчас</h2>
...
.hero-title   { animation: fade-up 0.7s ease 100ms both; }
.hero-text    { animation: fade-up 0.7s ease 250ms both; }
.hero-buttons { animation: fade-up 0.7s ease 400ms both; }
